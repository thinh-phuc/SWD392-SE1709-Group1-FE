...
export default function ScholarshipPage() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await BaseRequest.Delete(
        `/api/scholarships/${scholarshipToDelete.scholarshipId}`
      );
      // Refresh data
      ...
      const res = await BaseRequest.Get(
        `/api/scholarships?name=${name}&pageIndex=${page - 1}&pageSize=${limit}`
      );
      setData(res.items);
  ...
  const handleUpdateScholarship = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await BaseRequest.Put(
        `/api/scholarships/${selectedScholarship.scholarshipId}`,
        updatePayload
      );
      // Refresh data
      const page = Number(searchParams.get('page') || '1');
  ...
  const handleCreateScholarship = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await BaseRequest.Post('/api/scholarships', createForm);
      // Refresh data
      const page = Number(searchParams.get('page') || '1');
      const limit = searchParams.get('limit') || '10';
      const name = searchParams.get('keyword') || '';
      ...
      setCreateForm({ name: '', description: '', note: '', isActive: true });
    } catch (error) {
  ...
  const headers = [
    { accessorKey: 'scholarshipId', header: 'ID' },
    ...
    { accessorKey: 'note', header: 'Note' },
    {
      ...
      header: 'Is Active',
      cell: (info: CellContext<Scholarship, unknown>) =>
    ...
    {
      header: 'Actions',
      cell: ({ row }: CellContext<Scholarship, unknown>) => (
    ...
    <div>
      ...
      {loading ? (
        ...
        <DataTable
          ...
          pageCount={pageCount}
          showSearch={true}
          placeHolderInputSearch="Searching scholarships..."
        />
      )}
      <GenericModal
        open={showEditModal}
        ...
        description="Update scholarship details here"
      >
        <form
      ...
      <GenericModal
        ...
        title="Xác nhận xóa"
        description={`Bạn có chắc chắn muốn xóa học bổng "${scholarshipToDelete?.name}"?`}
      >
      ...
      <GenericModal
        ...
        title="Tạo mới học bổng"
        description="Nhập thông tin học bổng mới"
      >
        <form
